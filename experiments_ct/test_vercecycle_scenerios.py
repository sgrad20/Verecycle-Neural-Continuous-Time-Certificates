...
def make_plots(rows, out_dir: Path):
    ...
    plt.figure(figsize=(10, 5))
    plt.bar(x - 0.2, m_lb, width=0.4, label="IBP lower bound")
    plt.bar(x + 0.2, m_grid, width=0.4, label="Grid-based estimate")
    plt.xticks(x, names, rotation=20, ha="right")
    plt.ylabel("Certificate value")
    plt.title("IBP lower bound vs grid-based estimate")
    plt.legend()
    plt.tight_layout()
